...
app = FastAPI(title="assembly-service", lifespan=lifespan)
...
async def start_job(req: StartJobRequest):
    from job_handler import run_assembly_job
    job = await job_manager.submit(
        req.project_id,
        lambda job: run_assembly_job(job, req.project_id, req.config.model_dump()),
    )
