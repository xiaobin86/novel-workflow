...
> ⚠️ **文档与代码同步状态**：本文档部分描述与当前代码不一致，详见文末「文档与代码不一致清单」。以代码为准。
...
│   └── api/                          # API Routes（见第3节）
├── components/                       # ⚠️ 所有组件都在根目录，无子目录
│   ├── step-artifacts.tsx            # 产物预览（5个步骤的展示组件）
...
│   └── useStepControl.ts             # 步骤控制（⚠️ 仅 stop，无 pause/resume）
├── lib/
│   ├── services.ts                   # 各服务端口/地址映射
│   ├── project-store.ts              # 项目 JSON 文件读写 + 状态验证
...
> ⚠️ **实际代码与早期设计的差异**：
> - 所有组件**扁平化**在 `components/` 根目录，无 `project/`、`pipeline/`、`viewer/` 子目录
> - `StepCard`、`useAutoMode` 等并非独立文件，而是**内联在 `app/projects/[id]/page.tsx`** 中（详见该文件）
> - `pause/` 和 `resume/` API 路由**不存在**，仅实现了 `start/`、`stop/`、`reset/`、`regenerate-item/`、`events/`
...
⚠️ **非独立组件**，实际代码中作为**内联组件**定义在 `app/projects/[id]/page.tsx` 中（约第 180 行起）。
...
- `in_progress`：进度条 + `[■ 停止]` 按钮
- `stopped`：已停止提示 + `[重新开始]` 按钮（利用断点续传）
...
⚠️ **非独立 hook**，实际代码中作为**内联 hook** 定义在 `app/projects/[id]/page.tsx` 中（约第 21-29 行）。
...
function useStepProgress(projectId: string, step: StepName, active: boolean): StepProgress {
  ...
  return { events, lastEvent, isComplete, isStopped, error, percent };
}
```
...
│ E01_001  旁白  ▶ [─────────────────] 5.3s            │
    ...
    **AudioList 组件**：
    - 音频路径：`/api/projects/{project_id}/files/audio/{shot_id}_{track}.mp3`（edge-tts 输出 mp3）
...
```
# 本地开发模式（Windows）
PROJECTS_BASE_DIR=D:/work/novel-workflow/projects
...
ASSEMBLY_SERVICE_URL=http://localhost:8005

# Docker 模式（Linux/macOS）
